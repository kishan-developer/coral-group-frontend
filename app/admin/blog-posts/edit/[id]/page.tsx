...
interface BlogPost {
  ...
  notes?: string;
  followup?: string;
  queryStatus?: 'pending' | 'in-progress' | 'resolved' | 'closed';
...
export default function EditBlogPostPage() {
  ...
  const fetchPost = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/blog-posts/${params.id}`, {
        headers: {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          queryStatus: data.data.queryStatus || 'pending',
          priority: data.data.priority || 'medium',
        });
        setFeaturedImage(data.data.featuredImage || null);
          ...
          try {
            ...
            const fixedBlocks = blocks.map((block: any) => {
              if (block.type === 'text') {
                ...
                console.log('Image block loaded:', block);
              }
              return block;
            });
